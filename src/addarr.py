...
SERIE_MOVIE_AUTHENTICATED, READ_CHOICE, GIVE_OPTION, GIVE_PATHS, TSL_NORMAL, GIVE_QUALITY_PROFILES, GIVE_SEASONS = range(
    7)
    ...
    deleteMovieserie_handler = ConversationHandler(
        ...
        states={
            ...
            READ_DELETE_CHOICE: [
                ...
                CallbackQueryHandler(delete.confirmDelete,
                                     pattern=f'^({i18n.t("addarr.Movie")}|{i18n.t("addarr.Series")})$')
            ],
            GIVE_OPTION: [
    ...
    addMovieserie_handler = ConversationHandler(
        ...
        states={
            ...
            READ_CHOICE: [
                ...
                CallbackQueryHandler(searchSerieMovie,
                                     pattern=f'^({i18n.t("addarr.Movie")}|{i18n.t("addarr.Series")})$'),
                MessageHandler(
                    filters.Regex(f'^({i18n.t("addarr.New")})$'),
                    startSerieMovie
...
def addSerieMovie(update, context):
    ...
    if not service.inLibrary(idnumber):
        ...
        if added:
            ...
            if choice == i18n.t("addarr.Movie"):
                message = i18n.t("addarr.messages.AddFailed",
                                 subjectWithArticle=i18n.t("addarr.MovieWithArticle").lower())
            else:
                message = i18n.t("addarr.messages.AddFailed",
...
def clearUserData(context):
    ...
    for x in [
        x
        for x in
        ["choice", "title", "position", "output", "paths", "path", "qualityProfiles", "qualityProfile", "update_msg",
         "title_update_msg", "photo_update_msg", "selectedSeasons", "seasons"]
